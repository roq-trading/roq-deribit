...
struct ROQ_PUBLIC Config final : public server::config::Dispatcher, public server::config::Reader::Handler {
  ...
  std::string const &get_access_secret(Account const &) const;
...
template <>
struct fmt::formatter<roq::deribit::gateway::Config> {
  constexpr auto parse(format_parse_context &context) { return std::begin(context); }
  auto format(roq::deribit::gateway::Config const &value, format_context &context) const {
