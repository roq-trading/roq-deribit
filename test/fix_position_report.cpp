...
using PositionReport = deribit::protocol::fix::PositionReport;
...
TEST_CASE("fix_position_report_parse_message", "[fix_position_report]") {
  ...
  auto parser = [&](auto &message_2) {
    ++results;
    CHECK(message_2.header.msg_type == fix::MsgType::POSITION_REPORT);
    ...
    CHECK(position_report.pos_req_type == fix::PosReqType::POSITIONS);
    CHECK(position_report.pos_req_result == fix::PosReqResult::VALID);
    ...
    CHECK(item.qty_type == fix::QtyType::CONTRACTS);
