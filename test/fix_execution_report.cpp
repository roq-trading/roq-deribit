...
TEST(fix_execution_report, parse_message) {
  ...
  auto bytes = core::fix::Reader<core::fix::Version::FIX_44>::dispatch(
      [&](const core::fix::message_t &message) {
        ...
        EXPECT_EQ(result.order_id, "2831903667"_sv);
        EXPECT_EQ(result.cl_ord_id, "2831903667"_sv);
        ...
        EXPECT_EQ(result.text, "success"_sv);
        EXPECT_EQ(result.security_exchange, "DERIBITSERVER"_sv);
...
TEST(fix_execution_report, parse_order_mass_status) {
  const auto message =
      ...
      "85=7\00158=total_reports\001911=1\00110=045\001"_sv;
  core::Buffer buffer(4096);
  core::fix::Buffer decode_buffer(buffer);
  int results = 0;
  auto bytes = core::fix::Reader<core::fix::Version::FIX_44>::dispatch(
      [&](const core::fix::message_t &message) {
        ...
        EXPECT_EQ(message.header.msg_type, core::fix::MsgType::EXECUTION_REPORT);
        auto result = fix::ExecutionReport::create(message, decode_buffer);
        EXPECT_EQ(result.mass_status_req_id, "roq-oms-005"_sv);
...
TEST(fix_execution_report, parse_fill) {
  const auto message =
      ...
      "00\0011365=1.0000\0011443=1\00110=177\001"_sv;
  core::Buffer buffer(4096);
  core::fix::Buffer decode_buffer(buffer);
  int results = 0;
  auto bytes = core::fix::Reader<core::fix::Version::FIX_44>::dispatch(
      [&](const core::fix::message_t &message) {
        ...
        EXPECT_EQ(message.header.msg_type, core::fix::MsgType::EXECUTION_REPORT);
        auto result = fix::ExecutionReport::create(message, decode_buffer);
        EXPECT_EQ(result.order_id, "3026811591"_sv);
        EXPECT_EQ(result.cl_ord_id, "3026811591"_sv);
        EXPECT_EQ(result.orig_cl_ord_id, "roq:000000014"_sv);
        EXPECT_EQ(result.exec_type, core::fix::ExecType::ORDER_STATUS);
        ...
        EXPECT_EQ(result.ord_rej_reason, core::fix::OrdRejReason::BROKER_EXCHANGE_OPTION);
        EXPECT_EQ(result.text, "notification"_sv);
        EXPECT_EQ(result.security_exchange, "DERIBITSERVER"_sv);
        EXPECT_EQ(result.symbol, "BTC-27DEC19"_sv);
        EXPECT_EQ(result.qty_type, core::fix::QtyType::CONTRACTS);
        EXPECT_DOUBLE_EQ(result.contract_multiplier, 10.0);
        EXPECT_DOUBLE_EQ(result.avg_px, 9593.504);  // TODO(thraneh): why different? not just
                                                    // the commission...
        EXPECT_DOUBLE_EQ(result.max_show, 1.0);
        EXPECT_EQ(result.deribit_label, "roq:1:1:1000"_sv);
        ...
        EXPECT_EQ(item_0.fill_exec_id, "BTC-27DEC19#2350428"_sv);
